...
func (a *apiFeature) theResponseShouldMatchJSON(body *godog.DocString) (err error) {
	...
	// Unmarshal expected response
	if err = json.Unmarshal([]byte(body.Content), &expected); err != nil {
		return
	}
	// Unmarshal actual response too
	if err = json.Unmarshal(a.resp.Body.Bytes(), &actual); err != nil {
		return
	}
	// match the expected and actual result.
	if !reflect.DeepEqual(expected, actual) {
		return fmt.Errorf("expected result does not match actual, %v vs. %v", expected, actual)
	}
	return nil
}
